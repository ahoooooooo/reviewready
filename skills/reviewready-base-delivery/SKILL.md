...
complete/passed proof must have `closed=true` and a known host `previousStatus`
(including `running` when the report arrived before close). An observing worker
has no close proof yet. A complete final reviewer assignment in the handoff must also carry that exact
