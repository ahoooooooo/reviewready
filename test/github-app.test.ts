...
function tokenPolicy(overrides: Partial<GitHubAppTokenPolicy> = {}): GitHubAppTokenPolicy {
  return {
...
describe("GitHub App authentication", () => {
  ...
  it("requests and validates an installation token without logging credentials", async () => {
    ...
    await expect(
      createInstallationAccessToken(request, { appId: 123, privateKey, nowMs }, 17, tokenPolicy())
  ...
  it("rejects a write-capable installation token", async () => {
    ...
    await expect(
      createInstallationAccessToken(request, credentials(), 17, tokenPolicy())
    ).rejects.toMatchObject({
      code: "GITHUB_APP_AUTH_INVALID"
    });
  });

  it("rejects installation and repository scope outside the explicit policy", async () => {
    const policy = {
      allowedInstallationIds: [17],
      allowedRepositoryIds: [99],
      requiredPermissions: { metadata: "read", contents: "read" }
    ...
    const issueToken = createInstallationAccessToken as unknown as (
      client: GitHubAppRequest,
      appCredentials: ReturnType<typeof credentials>,
      installationId: number,
      tokenPolicy: unknown
    ) => Promise<unknown>;

    await expect(issueToken(request, credentials(), 18, policy)).rejects.toMatchObject({
      code: "GITHUB_APP_AUTH_INVALID"
    });
  });
  ...
  it("fails closed for invalid credentials, clock, installation, and transport input", async () => {
    ...
    await expect(
      createInstallationAccessToken(responseRequest({}), credentials(), 0, tokenPolicy())
    ).rejects.toMatchObject({ code: "GITHUB_APP_AUTH_INVALID" });
    await expect(
      createInstallationAccessToken(
        responseRequest({}),
        null as unknown as Parameters<typeof createInstallationAccessToken>[1],
        17,
        tokenPolicy()
      )
    ).rejects.toMatchObject({ code: "GITHUB_APP_AUTH_INVALID" });
    await expect(
      createInstallationAccessToken(
        ...
        credentials(),
        17,
        null as unknown as Parameters<typeof createInstallationAccessToken>[3]
      )
    ).rejects.toMatchObject({ code: "GITHUB_APP_AUTH_INVALID" });
    ...
    await expect(
      createInstallationAccessToken(failing, credentials(), 17, tokenPolicy())
    ).rejects.toMatchObject({
  ...
  it.each([
    ...
    await expect(
      createInstallationAccessToken(responseRequest(data), credentials(), 17, tokenPolicy())
    ).rejects.toMatchObject({ code: "GITHUB_APP_AUTH_INVALID" });
  });
  ...
  it("fails closed for invalid installation responses and private-key input", async () => {
    ...
    await expect(
      createInstallationAccessToken(
        request,
        { appId: 123, privateKey: "not-a-key", nowMs },
        17,
        tokenPolicy()
      )
    ).rejects.toMatchObject({ code: "GITHUB_APP_AUTH_INVALID" });
