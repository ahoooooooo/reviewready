...
import type { EvaluationBinding } from "../src/trust.js";
...
function signedRequest(
  ...
  return {
    ...
    receivedAtMs: overrides.receivedAtMs ?? 1_000
  };
}
...
describe("GitHub webhook ingress", () => {
  it("verifies raw bytes and atomically records a namespaced delivery", async () => {
    ...
    const result = await receiveGitHubWebhook(
      { rawBody, headers: headers(rawBody), receivedAtMs: 1_000 },
      { appId: 123, webhookSecret: secret },
      durable
    );
  ...
  it("rejects unsupported actions, duplicate headers, and bad signatures before durable access", async () => {
    ...
    await expect(
      receiveGitHubWebhook(
        { rawBody, headers: invalidHeaders, receivedAtMs: 1_000 },
        { appId: 123, webhookSecret: secret },
        durable
      )
  ...
  it("lets the durable namespace reject a replayed body under a new delivery id", async () => {
    ...
    await expect(
      receiveGitHubWebhook(
        { rawBody, headers: headers(rawBody, "delivery-1"), receivedAtMs: 1_000 },
        { appId: 123, webhookSecret: secret },
        durable
      )
    ...
    await expect(
      receiveGitHubWebhook(
        { rawBody, headers: headers(rawBody, "delivery-2"), receivedAtMs: 1_001 },
        { appId: 123, webhookSecret: secret },
        durable
      )
    ).resolves.toMatchObject({ accepted: false, reason: "duplicate" });
  ...
  it("does not throw for inherited or unknown event names", async () => {
    ...
    await expect(
      receiveGitHubWebhook(
        { rawBody, headers: headers(rawBody, "delivery-1", "toString"), receivedAtMs: 1_000 },
        { appId: 123, webhookSecret: secret },
        store()
      )
    ).resolves.toMatchObject({ accepted: false, reason: "invalid" });
  ...
  it("fails closed for malformed request framing and payloads", async () => {
    ...
    const cases: Array<{
      ...
      readonly config?: { readonly appId: number; readonly webhookSecret: string };
    }> = [
      { rawBody: new Uint8Array(MAX_WEBHOOK_BODY_BYTES + 1), request: {} },
      { rawBody, request: { receivedAtMs: -1 } },
      { rawBody, request: {}, config: { appId: 0, webhookSecret: secret } },
      { rawBody, request: {}, config: { appId: 123, webhookSecret: 7 as unknown as string } },
    ...
    for (const candidate of cases) {
      await expect(
        receiveGitHubWebhook(
          ...
          candidate.config ?? { appId: 123, webhookSecret: secret },
          store()
        )
      ).resolves.toMatchObject({ accepted: false, reason: "invalid" });
    }
  ...
  it("fails closed for invalid payload identity, action, SHA, and expiry", async () => {
    ...
    for (const payload of invalidPayloads) {
      ...
      await expect(
        receiveGitHubWebhook(signedRequest(rawBody), { appId: 123, webhookSecret: secret }, store())
      ).resolves.toMatchObject({ accepted: false, reason: "invalid" });
    }
    await expect(
      receiveGitHubWebhook(
  ...
  it("accepts a review event only with its review discriminator and reports store failures", async () => {
    ...
    await expect(
      receiveGitHubWebhook(
        { rawBody, headers: reviewHeaders, receivedAtMs: 1_000 },
        { appId: 123, webhookSecret: secret },
        store("new")
      )
